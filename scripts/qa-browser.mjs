...
const mime = { '.html':'text/html; charset=utf-8', '.css':'text/css', '.js':'text/javascript', '.json':'application/json', '.svg':'image/svg+xml', '.webp':'image/webp', '.jpg':'image/jpeg', '.png':'image/png' };
...

{
  ...
  for (const required of ['QA%20Test','6515550100','St.%20Paul','Garage%20door%20will%20not%20open']) if (!opened.includes(required)) fail(`Request Service handoff missing ${required}`);
  await context.close();
}
